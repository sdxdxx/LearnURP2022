Shader "Debug/URP_GeometryTBNLines_ModifiedTest"
{
    Properties
    {
        [Header(TBN Toggles)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            VertexOutput vert (Attributes input)
            {
                ...
                float3 normalTS = float3(input.uv2.xy, input.uv3.x) * 2.0 - 1.0;

                float3 T = normalize(input.tangentOS.xyz);
                float3 N = normalize(input.normalOS.xyz);
                float3 B = normalize(cross(N, T) * (input.uv3.y*2.0-1.0));

                // 获取世界空间的 TBN
                float3 normalOS = normalize(normalTS.x * T + normalTS.y * B + normalTS.z * N);
                //normalOS = input.color*2.0-1.0;
                //normalOS = float3(input.uv2.xy, input.uv3.x)*2.0-1.0;
                output.normalWS = TransformObjectToWorldNormal(normalOS);
                output.tangentWS = TransformObjectToWorldDir(input.tangentOS.xyz);
                output.bitangentWS = TransformObjectToWorldDir(B);
