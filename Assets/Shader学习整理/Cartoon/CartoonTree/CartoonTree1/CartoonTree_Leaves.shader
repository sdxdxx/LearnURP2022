    	...
    	HLSLINCLUDE
            ...
            half4 frag (vertexOutput i) : SV_TARGET
            {
				...
				for (uint lightIndex = 0; lightIndex < lightCount; lightIndex++)
				{
					...
					float additional_nDotl = dot(nDirWS,additionalLightDir);
					float additionalLambert = lerp(0.2,1,max(0,additional_nDotl));
					float3 perDiffuse = additionalLambert*additionalLightColor*additionalLightRadiance;
				    additionalDiffuse += perDiffuse;
				}
            	
            	//diffuse
                half3 diffuse= lerp(albedo*_DarkColor,albedo,halfLambert)+additionalDiffuse*_TopColor;
            	...
            	half3 FinalRGB = saturate((diffuse+specCol)*mainLight.color+_MainLightColor*scattering);
