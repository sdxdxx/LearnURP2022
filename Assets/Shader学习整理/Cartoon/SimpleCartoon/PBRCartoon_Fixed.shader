Shader "URP/Cartoon/PBRCartoon_Fixed"
{
    Properties
    {
    	[Header(Main Layer)]
    ...
    SubShader
    {
    	...
    	HLSLINCLUDE
    		...
    		inline half CalculateHatch(half4 rgba, half t)
			{
			    t = frac(t);                // wrap 到 [0,1)
			    half seg = t * 5.0h;        // 0..4
			    ...
			    half4 result = 0;
			    if (seg < pointA) result = lerp(0, rgba.r, smoothstep(0,pointA,seg)); // r->g
			    else if (seg < pointB) result = lerp(rgba.r, rgba.g, smoothstep(pointA,pointB,seg)); // g->b
			    else if (seg < pointC) result = lerp(rgba.g, rgba.b, smoothstep(pointB,pointC,seg)); // b->a
            	else if (seg < pointD) result = lerp(rgba.b, rgba.a, smoothstep(pointC,pointD,seg));
			    else result = rgba.a; // a->r
            ...
            half4 frag (vertexOutput i) : SV_TARGET
            {
            	...
            	//Cartoon
            	half halfLambert = dot(nDir,mainLightDir)*0.5f+0.5f;
            	float diffuse = halfLambert;
                ...
                half3 diffuseColor1 = albedo*mask1*half3(0.2829f,0.2976f,0.3584f);
                half3 diffuseColor2 = albedo*mask2*half3(0.4024f,0.4244f,0.4811f);
                half3 diffuseColor3 = albedo*mask3*half3(0.4396f,0.4660f,0.5377f);
                half3 diffuseColor4 = albedo*mask4*half3(0.6031f,0.6451f,0.7641f);
                half3 diffuseColor5 = albedo*mask5*half3(0.6776f,0.7272f,0.8584f);
                half3 diffuseColor6 = albedo*mask6;
            	albedo.rgb = diffuseColor1+diffuseColor2+diffuseColor3+diffuseColor4+diffuseColor5+diffuseColor6;
