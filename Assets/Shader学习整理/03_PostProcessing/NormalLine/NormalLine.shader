    ...
    SubShader
    {
        ...
        pass
        {
           ...
           #include "Packages/com.unity.render-pipelines.core/ShaderLibrary/SpaceTransforms.hlsl"
            ...
            half3 CalculateDepthNormalMapEdge(float2 screenPos)
            {
                ...
                for (int i = 0; i<4; i++)
                {
                    ...
                    float3 nDirWS_sample = SAMPLE_TEXTURE2D(_CameraNormalsTexture, sampler_PointClamp, sampleScreenPos);
                    ...
                    if ((abs(rawDepth-sampleRawDepth)>0.0011))
                    {
                        return half3(0,0,1);
                    }
                    
                    if (abs(mc_00-mc_sample)>0.1)
                    {
                        return half3(1,0,0);
                    }
            ...
            half4 frag (Varyings i) : SV_TARGET
            {
                ...
                half3 nomralLine = CalculateDepthNormalMapEdge(i.texcoord);
                float3 nDirWS_00 = SAMPLE_TEXTURE2D(_CameraNormalsTexture, sampler_PointClamp, i.texcoord);
                half4 result = half4(nomralLine.rgb,1.0);
                return result;
            }
