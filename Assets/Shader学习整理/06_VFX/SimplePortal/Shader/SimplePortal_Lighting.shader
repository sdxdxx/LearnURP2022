Shader "URP/SimplePortal/Lighting"
{
    Properties
    {
        [Header(MainMask)]
        ...
        _MainMaskPow("Main Mask Power",Range(0,10)) = 2.56
        _SmoothMainMaskPow("Smooth Main Mask Power",Range(0,10)) = 1
        _SmoothMainMaskBottom("SmoothMainMaskBottom",Range(0,0.2)) = 0.1
        _FlowSpeed("(XY) UV Flow",Vector) = (0,-1,0,0)
        
        [Header(Rotation)]
        _RotationSpeed("Rotation Speed",float) = 1
        _RotationAxis("(Local) Rotation Axis",Vector) = (0,0,1,0)
        _RotationOffset("Rotation Offset",Range(-1,1)) = 0
        
        [Header(Distortion)]
        _DistortionMap("Distortion Map",2D) = "Black"{}
        _DistortionPow("Distortion Power",float) = 1
        _DistortionSpeed("(XY) Distortion Speed",Vector) = (0,0,1,0)
    ...
    SubShader
    {
         ...
         pass
        {
            ...
            float4 _MainMask_ST;
            float _MainMaskPow;
            float _SmoothMainMaskPow;
            float _SmoothMainMaskBottom;
            float2 _FlowSpeed;
           ...
           float3x3 AngleAxis3x3(float angle, float3 axis)
           {
            ...
            float4 frag (vertexOutput i) : SV_TARGET
            {
                ...
                float2 distortionMapUV = i.uv*_DistortionMap_ST.xy+_DistortionMap_ST.zw;
                distortionMapUV = distortionMapUV + _DistortionSpeed.xy*_Time.y;
                float3 distortion = SAMPLE_TEXTURE2D(_DistortionMap,sampler_DistortionMap,distortionMapUV);
                float2 maskUV = lerp(i.uv,i.uv+distortion,_DistortionAmount)*_MainMask_ST.xy+_MainMask_ST.zw;
                maskUV = maskUV+_FlowSpeed*_Time.y;
