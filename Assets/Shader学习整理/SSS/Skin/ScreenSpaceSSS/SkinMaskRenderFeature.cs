...
public class SkinMaskRenderFeature : ScriptableRendererFeature
{
    [System.Serializable]
    ...
    class CustomRenderPass : ScriptableRenderPass
    {
        ...
        private const string ProfilerTag = "SkinMask";
        private ProfilingSampler m_ProfilingSampler = new(ProfilerTag);
        
        private Material material;

        private ScreenSpaceSSS_SkinVolume skinVolume;
        ...
        public CustomRenderPass(Settings settings)
        {
            ...
            filtering = new FilteringSettings(RenderQueueRange.all, settings.skinLayer);//设置过滤器
            //shaderTagsList.Add(new ShaderTagId("SRPDefaultUnlit"));
            //shaderTagsList.Add(new ShaderTagId("UniversalForward"));
            //shaderTagsList.Add(new ShaderTagId("UniversalForwardOnly"));
            shaderTagsList.Add(new ShaderTagId("SkinMask"));
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            skinVolume = stack.GetComponent<ScreenSpaceSSS_SkinVolume>();//从栈中获取到ColorTintVolume
            material.SetColor("_SSSColor", skinVolume.SSSColor.value);//将材质颜色设置为volume中的值
            material.SetFloat("_SSSIntensity",skinVolume.SSSIntensity.value);
            ...
            using (new ProfilingScope(cmd, m_ProfilingSampler))
            {
                ...
                material.SetTexture("_SkinMask",tempRTHandle3);

                ConfigureTarget(cameraColorRTHandle);//确认传入的目标为cameraColorRT

                Blitter.BlitCameraTexture(cmd,cameraColorRTHandle,tempRTHandle);
                for (int i = 0; i < skinVolume.Iteration.value; i++)
                {
                    material.SetVector ("_offsets", new Vector4 (0, skinVolume.BlurRadius.value, 0, 0));
                    Blitter.BlitCameraTexture(cmd,tempRTHandle,tempRTHandle2,material,1);
                    material.SetVector ("_offsets", new Vector4 (skinVolume.BlurRadius.value, 0, 0, 0));
                    Blitter.BlitCameraTexture(cmd,tempRTHandle2,tempRTHandle,material,1);
                    material.SetTexture("_GaussianSkinTex",tempRTHandle);
                    
                }
                Blitter.BlitCameraTexture(cmd,cameraColorRTHandle,cameraColorRTHandle,material,2);
