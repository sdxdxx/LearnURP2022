Shader "URP/SSS/MyPBR_Skin_ScreenSpace/"
{
    Properties
    {
    	[Header(Main Layer)]
    	...
    	[Header(AO)]
    	_AmbientOcculusionMap("Ambient Occulusion Map",2D) = "white"{}
    ...
    SubShader
    {
    	...
    	HLSLINCLUDE
    		...
    		TEXTURE2D(_AmbientOcculusionMap);
            SAMPLER(sampler_AmbientOcculusionMap);
            ...
            half4 frag (vertexOutput i) : SV_TARGET
            {
            	...
            	half3 sssColor = SAMPLE_TEXTURE2D(_SSSLut,sampler_SSSLut,float2(halfLambert,_SSSPow));

            	//Translucency
            	float translucency = SAMPLE_TEXTURE2D(_TranslucencyMap,sampler_TranslucencyMap,i.uv).r;

            	//AO
            	float ao = SAMPLE_TEXTURE2D(_AmbientOcculusionMap,sampler_AmbientOcculusionMap,i.uv).r;
            	
            	half3 FinalRGB = result_RBR*ao+translucency*sssColor*mainTex;
    	...
    	pass
        {
        	...
        	Tags{"LightMode" = "SkinMask"}

            HLSLPROGRAM
            
            #pragma vertex vert
            #pragma fragment frag_SkinMask

            half4 frag_SkinMask (vertexOutput i) : SV_TARGET
            {
