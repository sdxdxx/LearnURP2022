Shader "Ciconia Studio/CS_Skin/Builtin/SSS Skin (Specular setup) Cutout"
{
	Properties
	{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float4 tex2DNode8_g883 = tex2D( _CutoutMap, uv_CutoutMap );
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float2 break26_g864 = float4( uv_MainTex, 0.0 , 0.0 ).xy;
			float GlobalTilingX11 = ( _GlobalXYTilingXYZWOffsetXY.x - 1.0 );
			float GlobalTilingY8 = ( _GlobalXYTilingXYZWOffsetXY.y - 1.0 );
			float2 appendResult14_g864 = (float2(( break26_g864.x * GlobalTilingX11 ) , ( break26_g864.y * GlobalTilingY8 )));
			float GlobalOffsetX10 = _GlobalXYTilingXYZWOffsetXY.z;
			float GlobalOffsetY9 = _GlobalXYTilingXYZWOffsetXY.w;
			float2 appendResult13_g864 = (float2(( break26_g864.x + GlobalOffsetX10 ) , ( break26_g864.y + GlobalOffsetY9 )));
			...
			float4 tex2DNode7_g863 = tex2D( _MainTex, ( ( appendResult14_g864 + appendResult13_g864 ) + Parallaxe375 ) );
			float BaseColorAlpha46 = (( _InvertABaseColor )?( ( 1.0 - tex2DNode7_g863.a ) ):( tex2DNode7_g863.a ));
			float4 temp_cast_4 = (BaseColorAlpha46).xxxx;
			float4 temp_cast_5 = (BaseColorAlpha46).xxxx;
			float4 clampResult24_g883 = clamp( (( _InvertCutout )?( ( 1.0 - (( _UseBaseColorAlpha )?( temp_cast_5 ):( tex2DNode8_g883 )) ) ):( (( _UseBaseColorAlpha )?( temp_cast_4 ):( tex2DNode8_g883 )) )) , float4( 0,0,0,0 ) , float4( 1,0,0,0 ) );
			SurfaceOutputStandardSpecular s542 = (SurfaceOutputStandardSpecular ) 0;
			float4 lerpResult56_g863 = lerp( _Color , ( ( _Color * tex2DNode7_g863 ) * _Color.a ) , _Color.a);
			float clampResult27_g863 = clamp( _Saturation , -1.0 , 100.0 );
			float3 desaturateInitialColor29_g863 = lerpResult56_g863.rgb;
			float desaturateDot29_g863 = dot( desaturateInitialColor29_g863, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar29_g863 = lerp( desaturateInitialColor29_g863, desaturateDot29_g863.xxx, -clampResult27_g863 );
			float clampResult11_g866 = clamp( _ContrastSkin , 0.0 , 1000.0 );
			...
			#endif //aseld
			float dotResult4_g866 = dot( (WorldNormalVector( i , float3(0,0,1) )) , ase_worldlightDir );
			...
			float clampResult9_g866 = clamp( _SkinFill , 0.0 , 1000.0 );
			float4 RGBBaseColor640 = tex2DNode7_g863;
			float4 temp_cast_12 = (1.0).xxxx;
			float4 tex2DNode106_g865 = tex2D( _TranslucencyMapRMaskA, i.uv_texcoord );
			float TranslucencyAlpha652 = tex2DNode106_g865.a;
			float4 temp_cast_13 = (TranslucencyAlpha652).xxxx;
			float4 blendOpSrc637 = ( ( ( _SkinTone * CalculateContrast(clampResult11_g866,float4( ( ( max( dotResult4_g866 , 0.0 ) * ( LightAttenuation661 * ase_lightColor.rgb ) ) + indirectDiffuse620 + clampResult9_g866 ) , 0.0 )) ) * RGBBaseColor640 ) * (( _ExcludeUseTranslucencyAlpha )?( temp_cast_13 ):( temp_cast_12 )) );
			float4 blendOpDest637 = CalculateContrast(_Brightness,float4( desaturateVar29_g863 , 0.0 ));
			float4 blendOpSrc636 = ( ( ( _SkinTone * CalculateContrast(clampResult11_g866,float4( ( ( max( dotResult4_g866 , 0.0 ) * ( LightAttenuation661 * ase_lightColor.rgb ) ) + indirectDiffuse620 + clampResult9_g866 ) , 0.0 )) ) * RGBBaseColor640 ) * (( _ExcludeUseTranslucencyAlpha )?( temp_cast_13 ):( temp_cast_12 )) );
			float4 blendOpDest636 = CalculateContrast(_Brightness,float4( desaturateVar29_g863 , 0.0 ));
			float4 blendOpSrc634 = ( ( ( _SkinTone * CalculateContrast(clampResult11_g866,float4( ( ( max( dotResult4_g866 , 0.0 ) * ( LightAttenuation661 * ase_lightColor.rgb ) ) + indirectDiffuse620 + clampResult9_g866 ) , 0.0 )) ) * RGBBaseColor640 ) * (( _ExcludeUseTranslucencyAlpha )?( temp_cast_13 ):( temp_cast_12 )) );
			float4 blendOpDest634 = CalculateContrast(_Brightness,float4( desaturateVar29_g863 , 0.0 ));
				...
				float4 staticSwitch635 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc637 ) * ( 1.0 - blendOpDest637 ) ) ));
			#endif
			float4 lerpResult709 = lerp( CalculateContrast(_Brightness,float4( desaturateVar29_g863 , 0.0 )) , staticSwitch635 , _SkinBlend);
			float4 temp_cast_15 = (( (( _InvertTranslucencyMap )?( ( 1.0 - tex2DNode106_g865.r ) ):( tex2DNode106_g865.r )) + (-1.2 + (_SpreadTranslucency - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
			float4 clampResult126_g865 = clamp( CalculateContrast(_ContrastTranslucency,temp_cast_15) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 newWorldNormal99_g865 = normalize( (WorldNormalVector( i , float4( Normal27 , 0.0 ).xyz )) );
			float clampResult159_g865 = clamp( _NormalTranslucency , 0.0 , 1000.0 );
			float dotResult115_g865 = dot( ase_worldViewDir , -( ( newWorldNormal99_g865 * clampResult159_g865 ) + ase_worldlightDir ) );
			float saferPower169_g865 = abs( dotResult115_g865 );
			float clampResult160_g865 = clamp( _Power , 0.0 , 1000.0 );
			float dotResult161_g865 = dot( pow( saferPower169_g865 , 0.0 ) , clampResult160_g865 );
			float4 temp_output_137_0_g865 = CalculateContrast(_SaturateTraslucency,_TranslucencyColor);
			float3 desaturateInitialColor175_g865 = temp_output_137_0_g865.rgb;
			float desaturateDot175_g865 = dot( desaturateInitialColor175_g865, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar175_g865 = lerp( desaturateInitialColor175_g865, desaturateDot175_g865.xxx, _DesaturateAmbientTranslucency );
			float dotResult185_g865 = dot( ase_worldViewDir , -( newWorldNormal99_g865 + ase_worldlightDir ) );
			float saferPower120_g865 = abs( dotResult185_g865 );
			float clampResult122_g865 = clamp( _AmbientTranslucency , 0.0 , 1000.0 );
			float dotResult124_g865 = dot( pow( saferPower120_g865 , (0.0 + (_ThicknessTranslucency - 0.0) * (15.0 - 0.0) / (1.0 - 0.0)) ) , clampResult122_g865 );
			float TranslucencymapAlpha182_g865 = tex2DNode106_g865.a;
			float4 blendOpSrc148_g865 = ( ( ( clampResult126_g865 * _TranslucencyIntensity ) * saturate( ( dotResult115_g865 * dotResult161_g865 ) ) ) * temp_output_137_0_g865 );
			float4 blendOpDest148_g865 = float4( ( desaturateVar175_g865 * saturate( ( ( dotResult185_g865 * dotResult124_g865 ) * (( _MaskTranslucency )?( (( _InvertTranslucencyAlpha )?( ( 1.0 - TranslucencymapAlpha182_g865 ) ):( TranslucencymapAlpha182_g865 )) ):( 1.0 )) ) ) ) , 0.0 );
			float lerpResult152_g865 = lerp( 1.0 , LightAttenuation661 , _LightAttenuationTranslucency);
			float4 temp_output_138_0_g865 = ( ( saturate( ( blendOpSrc148_g865 + blendOpDest148_g865 ) )) * lerpResult152_g865 * (( _LightIntensityToggle )?( ase_lightColor.a ):( 1.0 )) );
			float4 VisualizeTranslucency400 = temp_output_138_0_g865;
			#ifdef _VISUALIZETRANSLUCENCY_ON
				float4 staticSwitch468 = VisualizeTranslucency400;
			#else
				float4 staticSwitch468 = (( _EnableSkinTone )?( lerpResult709 ):( CalculateContrast(_Brightness,float4( desaturateVar29_g863 , 0.0 )) ));
			#endif
			float4 Albedo26 = staticSwitch468;
			s542.Albedo = Albedo26.rgb;
			s542.Normal = WorldNormalVector( i , Normal27 );
			float2 uv_EmissionMap = i.uv_texcoord * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
			float2 break26_g872 = uv_EmissionMap;
			float2 appendResult14_g872 = (float2(( break26_g872.x * GlobalTilingX11 ) , ( break26_g872.y * GlobalTilingY8 )));
			float2 appendResult13_g872 = (float2(( break26_g872.x + GlobalOffsetX10 ) , ( break26_g872.y + GlobalOffsetY9 )));
			float4 temp_output_5_0_g871 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g872 + appendResult13_g872 ) + Parallaxe375 ) ) );
			float4 Emission110 = ( temp_output_5_0_g871 * _EmissionIntensity );
			s542.Emission = Emission110.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float2 break26_g868 = uv_SpecGlossMap;
			float2 appendResult14_g868 = (float2(( break26_g868.x * GlobalTilingX11 ) , ( break26_g868.y * GlobalTilingY8 )));
			float2 appendResult13_g868 = (float2(( break26_g868.x + GlobalOffsetX10 ) , ( break26_g868.y + GlobalOffsetY9 )));
			float4 tex2DNode3_g867 = tex2D( _SpecGlossMap, ( ( appendResult14_g868 + appendResult13_g868 ) + Parallaxe375 ) );
			float3 desaturateInitialColor508 = ( ( _SpecularColor * tex2DNode3_g867 ) * _SpecularIntensity ).rgb;
			...
			float fresnelNdotV42_g867 = dot( mul(ase_tangentToWorldFast,float4( Normal27 , 0.0 ).xyz), ase_worldViewDir );
			float fresnelNode42_g867 = ( _FresnelBias + ( _SpecularIntensity * _FresnelIntensity ) * pow( 1.0 - fresnelNdotV42_g867, _Fresnelpower ) );
			float clampResult38_g867 = clamp( fresnelNode42_g867 , 0.0 , 1.0 );
			float SpecularFresnel645 = clampResult38_g867;
			float3 temp_cast_26 = (SpecularFresnel645).xxx;
			float3 blendOpSrc646 = desaturateVar508;
			float3 blendOpDest646 = temp_cast_26;
			float3 temp_cast_27 = (0.0).xxx;
			#ifdef _VISUALIZETRANSLUCENCY_ON
				float3 staticSwitch469 = temp_cast_27;
			#else
			...
			s542.Specular = Specular41;
				...
				float staticSwitch31_g867 = ( tex2DNode3_g867.a * _Glossiness );
			#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch31_g867 = ( _Glossiness * BaseColorAlpha46 );
			#else
				float staticSwitch31_g867 = ( tex2DNode3_g867.a * _Glossiness );
			#endif
			float Smoothness40 = staticSwitch31_g867;
			s542.Smoothness = Smoothness40;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float2 break26_g870 = uv_OcclusionMap;
			float2 appendResult14_g870 = (float2(( break26_g870.x * GlobalTilingX11 ) , ( break26_g870.y * GlobalTilingY8 )));
			float2 appendResult13_g870 = (float2(( break26_g870.x + GlobalOffsetX10 ) , ( break26_g870.y + GlobalOffsetY9 )));
			float blendOpSrc2_g869 = tex2D( _OcclusionMap, ( ( appendResult14_g870 + appendResult13_g870 ) + Parallaxe375 ) ).r;
			float blendOpDest2_g869 = ( 1.0 - _AoIntensity );
			float AmbientOcclusion94 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g869 ) * ( 1.0 - blendOpDest2_g869 ) ) ));
			...
			surfResult542 -= s542.Emission;
			#endif//542
			float4 Translucency158 = temp_output_138_0_g865;
			c.rgb = ( float4( surfResult542 , 0.0 ) + Translucency158 ).rgb;
			c.a = 1;
			clip( clampResult24_g883.r - _MaskClip );
			return c;
		}
